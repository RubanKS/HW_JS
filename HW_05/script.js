...
for (let i = 0; i < Object.values(numbers).length; i++) {
	if (Object.values(numbers)[i] >= 3) {
		text = text + ' ' + Object.values(numbers)[i];
	}
}
console.log(text);
...
console.log(`${Object(post.author)}, ${Object(post.comments[0].rating.dislikes)},` +
	` ${Object(post.comments[1].userId)}, ${Object(post.comments[1].text)}`);
...
console.log(productsFour.filter(prod => 'photos' in prod && prod.photos.length !== 0));
console.log(productsFour.sort((prodFirst, prodSecond) => prodFirst.price - prodSecond.price));
...
const combArr = Object.fromEntries(en.map((k, i) => [k, ru[i]]));
console.log(combArr);